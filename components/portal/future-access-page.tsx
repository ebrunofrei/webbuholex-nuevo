...
export function FutureAccessPage({ eyebrow, title, description, status, boundaries, mode, loginAction }: { eyebrow: string; title: string; description: string; status: string; boundaries: readonly AccessBoundary[]; mode: "login" | "space"; loginAction?: React.ReactNode }) {
  ...
  return (
    <section className={styles.page} aria-labelledby="future-access-title"><div className={styles.container}>
      ...
      <div className={styles.actions}>{mode === "space" ? <><Link href="/iniciar-sesion/">INGRESAR</Link><Link href="/explorar/">VOLVER A INFORMACIÓN PÚBLICA</Link></> : <>{loginAction}<Link href="/explorar/">VOLVER A INFORMACIÓN PÚBLICA</Link><Link href="/espacio/">CONOCER EL ESPACIO INTELIGENTE</Link></>}</div>
