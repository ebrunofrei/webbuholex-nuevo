...
// - Devuelve array de �tems normalizados (para aggregator)
// ============================================================
...
async function fetchNoticias({
  ...
  // page se ignora: el listado de CIJ es est�tico (paginaci�n JS)
} = {}) {
  try {
    ...
    // Filtro temporal (por si el aggregator no lo aplic�)
    let items = out;
    if (since) {
    ...
    // Filtro por q (t�picos simples, coma separada)
    if (q && q.trim()) {
      const toks = q
        .split(",")
