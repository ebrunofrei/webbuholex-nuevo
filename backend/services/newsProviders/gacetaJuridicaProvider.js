...
function parseStaticList(html) {
  ...
  cards.each((_, el) => {
    ...
    // t�tulo + enlace (evita anchors vac�os)
    const $a = $el
...
async function fetchNoticias({
  ...
  try {
    ...
    // 4) b�squeda simple por q (t�tulo/resumen)
    if (q && q.trim()) {
    ...
    console.error("? Gaceta Jur�dica:", err?.message || err);
    return [];
  }
}
