...
async function fetchNoticias({
  ...
  limit = 12,
  lang = "es",
  since = null,           // recomendado para �m�x. 2 d�as�
  especialidad = "todas", // no se usa duro aqu�; el front/back superior puede filtrar
} = {}) {
  let browser;
  try {
    ...
    p.on("request", (req) => {
      ...
      if (type === "image" || type === "media" || type === "font") {
        // dejamos pasar algunas im�genes de portada para obtener src, pero en general bloqueamos
        // aqu� bloqueamos todo por simplicidad (mejor desempe�o en server)
        return req.abort();
      }
      req.continue();
    });
    ...
    // Algunas tarjetas cargan con m�s scroll
    await autoScroll(p);
    ...
    const rawItems = await p.evaluate(() => {
      ...
      // intenta recoger art�culos en distintas estructuras
    ...
    let items = rawItems.map((n) => {
      ...
      return normalizeItem({
        ...
        tipo: "juridica",
        especialidad: "administrativo", // etiqueta gen�rica; el front puede reclasificar por keywords
        lang: "es",
      });
    });
