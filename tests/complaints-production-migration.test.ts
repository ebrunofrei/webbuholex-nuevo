...
describe('Complaints Production Migration Guard', () => {
  ...
  describe('executeMigration', () => {
    it('aborts if marker nonexistent (missing marker)', async () => {
  ...
  describe('Static constraints (Code Analysis)', () => {
    it('production wrapper NO contiene INSERT/UPDATE/DELETE de environment_marker ni DDL ni bootstrap autocertificante', () => {
      const scriptPath = path.join(__dirname, '..', 'scripts', 'complaints-migrate-production.mjs');
      const content = fs.readFileSync(scriptPath, 'utf8');

      // The script should not contain DML against environment_marker
      expect(content).not.toMatch(/INSERT\s+INTO\s+.*environment_marker/i);
      ...
      // The script should not contain bootstrap logic
