...
import { z } from "zod";
import {
  authorizeAdminComplaintReview,
  StartComplaintReviewHttpSchema,
} from "@/lib/complaints/complaints-admin-http-runtime";
import { startComplaintReviewRuntime } from "@/lib/complaints/complaints-admin-runtime";
...
export async function POST(
  ...
  try {
    ...
    const parsed = StartComplaintReviewHttpSchema.safeParse(jsonBody);
    if (!parsed.success) {
      return NextResponse.json(
        { success: false, error: { code: "bad_request" } },
        { status: 400, headers: { "Cache-Control": "no-store", "Content-Type": "application/json" } }
