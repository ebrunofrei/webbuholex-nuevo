...
import { z } from "zod";
import {
  authorizeAdminComplaintResumeReview,
...
export async function POST(
  ...
  try {
    ...
    const authResult = await authorizeAdminComplaintResumeReview();
    ...
    const parsed = ResumeComplaintReviewHttpSchema.safeParse(jsonBody);
    if (!parsed.success) {
      return NextResponse.json(
        { success: false, error: { code: "bad_request" } },
        { status: 400, headers: { "Cache-Control": "no-store", "Content-Type": "application/json" } }
    ...
    switch (result.kind) {
      ...
      case "complaint_no_open_information_request":
      case "complaint_multiple_open_information_requests":
        return NextResponse.json(
          { success: false, error: { code: "conflict" } },
          { status: 409, headers: { "Cache-Control": "no-store", "Content-Type": "application/json" } }
        );
      case "complaint_resume_review_note_required":
      case "complaint_resume_review_note_too_long":
        return NextResponse.json(
