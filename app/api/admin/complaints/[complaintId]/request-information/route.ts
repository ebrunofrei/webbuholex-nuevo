...
import { z } from "zod";
import {
  authorizeAdminComplaintRequestInformation,
  RequestInformationHttpSchema,
} from "@/lib/complaints/complaints-admin-http-runtime";
import { requestComplaintInformationRuntime } from "@/lib/complaints/complaints-admin-runtime";
...
export async function POST(
  ...
  try {
    ...
    const authResult = await authorizeAdminComplaintRequestInformation();
    ...
    const complaintIdParsed = z.string().uuid().safeParse(complaintId);
    if (!complaintIdParsed.success) {
      return NextResponse.json(
    ...
    const parsed = RequestInformationHttpSchema.safeParse(jsonBody);
    if (!parsed.success) {
      return NextResponse.json(
        { success: false, error: { code: "bad_request" } },
        { status: 400, headers: { "Cache-Control": "no-store", "Content-Type": "application/json" } }
    ...
    const result = await requestComplaintInformationRuntime({
      complaintId,
    ...
    switch (result.kind) {
      ...
      case "complaint_open_information_request_exists":
      case "complaint_information_request_sequence_conflict":
        return NextResponse.json(
          { success: false, error: { code: "conflict" } },
          { status: 409, headers: { "Cache-Control": "no-store", "Content-Type": "application/json" } }
        );
      case "complaint_request_information_text_required":
      case "complaint_request_information_text_too_long":
        return NextResponse.json(
