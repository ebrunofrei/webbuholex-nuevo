...
export default function LitisBotBubbleChat({ usuarioId, pro = true }) {
  ...
  useEffect(() => {
    ...
    const move = (e) => {
      let x =
        (e.touches ? e.touches[0].clientX : e.clientX) - offset.current.x;
      let y =
        (e.touches ? e.touches[0].clientY : e.clientY) - offset.current.y;
      x = Math.max(10, Math.min(x, window.innerWidth - 80));
      y = Math.max(10, Math.min(y, window.innerHeight - (open ? 520 : 80)));
      setPos({ x, y });
    };
    const up = () => {
      setDrag(false);
  ...
  useEffect(() => {
    const fix = () =>
      setPos((prev) => ({
        ...
        y: Math.min(prev.y, window.innerHeight - (open ? 520 : 80)),
      }));
  ...
  return (
    <>
      ...
      {!open && (
        <div
          className="fixed z-[9999] cursor-move flex items-center justify-center"
          style={{
            left: pos.x,
            top: pos.y,
            width: 64,
      ...
      {open && (
        ...
        >
          {/* CABECERA */}
          <div className="flex items-center justify-between px-4 py-2 bg-[#6d4a28] text-white rounded-t-2xl">
