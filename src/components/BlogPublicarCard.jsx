...
export default function BlogPublicarCard({ onPublicado }) {
  ...
  const handleSubmit = async (e) => {
    ...
    if (form.portada) {
      urlPortada = await subirPortadaBlog(
        form.portada,
        `blog/portadas/${Date.now()}_${form.portada.name}`
      );
    }
  ...
  return (
    <form
      ...
      style={{ minWidth: 320 }}
    >
      <h2 className="text-2xl font-bold text-[#7a2518] mb-2 text-center">
        Publicar artículo
      </h2>
      ...
      <label className="block font-semibold text-[#3e2723]">
        ...
        <textarea
          ...
          onChange={handleChange}
          required
          className="w-full border rounded p-2 mt-1 min-h-[64px]"
          maxLength={250}
          placeholder="Escribe un resumen atractivo para el lector..."
        />
        <span className="block text-xs text-gray-500">
          {form.resumen.length}/250
        </span>
      </label>
      ...
      <label className="block font-semibold text-[#3e2723]">
        ...
        <input
          ...
          onChange={handleChange}
          className="w-full border rounded p-2 mt-1"
          placeholder="ej: civil, penal, procesal, IA"
        />
      </label>
      ...
      <label className="block font-semibold text-[#3e2723]">
        ...
        {preview && (
          <img
            src={preview}
            alt="portada"
            className="w-48 mx-auto mt-2 rounded-xl shadow border"
          />
        )}
      </label>
