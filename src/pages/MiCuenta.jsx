...
export default function MiCuenta() {
  ...
  // Callback después de confirmar pago manual (Yape/Plin)
  const handlePagoExitoso = async () => {
    toast({
      title: "¡Felicitaciones!",
      description: "Ahora eres usuario Premium.",
    });
    await reloadUsuario?.();
  };
  ...
  return (
    <div className="max-w-lg mx-auto mt-10 bg-white rounded-2xl shadow-lg p-8">
      ...
      <div className="flex flex-col items-center mb-6">
        ...
        />
        <div className="font-bold text-2xl text-[#4b2e19]">
          {user?.displayName || "Usuario anónimo"}
        </div>
        <div className="text-[#b03a1a] font-semibold">
          {user?.email || "Sin email"}
        </div>
        <div
          className={`font-bold mt-2 px-5 py-1 rounded-xl text-sm 
          ${
            isPremium
        ...
        {/* PAGO LOCAL: Yape/Plin */}
        {!isPremium && (
          <div className="mt-4 text-center">
            <h3 className="text-[#b03a1a] font-bold mb-2">
            ...
            <p className="text-sm text-gray-600 mt-2">
              Escanea el QR y envía el comprobante a:{" "}
              <span className="font-semibold">pagos@buholex.com</span>
            </p>
            <button
              onClick={handlePagoExitoso}
              className="mt-3 bg-[#b03a1a] text-white px-6 py-2 rounded-xl font-bold hover:bg-[#a52e00] transition"
            >
              Ya pagué
            </button>
          </div>
      ...
      <form onSubmit={handleEmailChange} className="mb-8">
        <label className="block mb-1 font-medium text-[#b03a1a]">
          Correo electrónico
        </label>
        <input
          type="email"
      ...
      <div className="mt-8 mb-2">
        <h3 className="text-[#b03a1a] font-bold text-lg mb-2">
